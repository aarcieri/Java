...
public class DES implements Icipher{
    ...
    private final int[] IP = { 58, 50, 42, 34, 26, 18,
    ...
    private final int[] IP1 = { 40, 8, 48, 16, 56, 24, 64,
                32, 39, 7, 47, 15, 55,
                23, 63, 31, 38, 6, 46,
                14, 54, 22, 62, 30, 37,
                5, 45, 13, 53, 21, 61,
    ...
    private final int[] PC1 = { 57, 49, 41, 33, 25,
    ...
    private final int[] PC2 = { 14, 17, 11, 24, 1, 5, 3,
    ...
    private final int[] EP = { 32, 1, 2, 3, 4, 5, 4,
                5, 6, 7, 8, 9, 8, 9, 10,
    ...
    private final int[] P = { 16, 7, 20, 21, 29, 12, 28,
                17, 1, 15, 23, 26, 5, 18,
                31, 10, 2, 8, 24, 14, 32,
                27, 3, 9, 19, 13, 30, 6,
                22, 11, 4, 25 };
    ...
    private final int[] shiftBits = { 1, 1, 2, 2, 2, 2, 2, 2,
                            1, 2, 2, 2, 2, 2, 2, 1 };

    
    
    
    // per-mutate input binary string,
...
//                            sbox[num][row][col]);
                    String tmp = Integer.toBinaryString(
                            sbox[num][row][col]);
                    if(tmp.length()<4) tmp = Utility.LeftPad(tmp, '0', 4);
                    output+=tmp;
            }
            return output;
    }

    private String iteration(String input, String key, int num)
    {
            // fk
...
//                                + key.toUpperCase());
    ...
    public String encrypt(String plainText, String key)
    {
            ...
            // convet to binary string
            plainText = Utility.toBinaryString(plainText);
            key = Utility.toBinaryString(key);
...
//                    + plainText.substring(8, 16).toUpperCase() + "\n");
            ...
            // 16 rounds
            for (i = 0; i < 16; i++) {
                    plainText = iteration(plainText, keys[i], i);
            }
            ...
            // convert to a characters
            plainText = Utility.toCharString(plainText);
...
//                    + "\n");
            ...
            // 16-rounds
            for (i = 15; i >=0; i--) {
                    plainText = iteration(plainText, keys[i], 15 - i);
            }
