...
public class AES implements Icipher{
    ...
    /**
     * S-BOX utilizzata per Key Expansion e per Sub-Bytes.
     */
    ...
    /**
     * Inversa di SBOX per invSubBytes
     */
    ...
    /**
     * tabella utilizzata per mixColumns
     */
    ...
    /**
     * RCon array utilizzato per Key Expansion
     */
    ...
    public int singleBlockDimension(){
        return 16; // 4 * 32 word = 128 bit = 16 byte
    }
    
    //copia due array bidimensionali
    ...
    //converte una matrice in una line di 32 caratteri esadecimali.
    ...
    public String encrypt(String plainText, String key){
        ...
        plainText = Utility.toHexString(plainText);
        key = Utility.toHexString(key);
        ...
        int[][] keymatrix = this.keySchedule(key);
        
        if (plainText.toUpperCase().matches("[0-9A-F]+")) 
        {
            if (plainText.length() < 32) {
                plainText = String.format("%1$32s", plainText).replace(' ', '0');
            }
            state = new int[4][4];
            
            // carica l'input nello stato
            for (int i = 0; i < 4; i++) 
            {
                for (int j = 0; j < 4; j++) {
            ...
            this.addRoundKey(state, this.subKey(keymatrix, 0)); 
            ...
            return Utility.toCharString(Utility.hextoBin(MatrixToString(state)));
        } 
    ...
    // verifica che la lunghezza della key, in esadecimale, sia valida
    // se non è valida spara una eccezione
    private void checkKeyLength(String key){
        int keyLenght = key.length() * 4;
        
        try 
        {
            //verifica se la chiave ha la dimensine giusta
            if(keyLenght!= 128 && keyLenght!= 192 && keyLenght!= 256) 
            {
                throw new Exception("Errore: imensione della chiave " + key.length() * 4 + "-bit non valida per AES");
            }           
        }
        catch (Exception e) 
        {
